...
# Description
# - 子プロセスを実行してその入力ストリームを管理するには subprocess モジュールを使う
# - 子プロセスは、Python インタプリタと並列に実行され、CPU利用を最大化することができる
# - 簡単な利用には便利な run 関数を使い、UNIX的なパイプラインのような高度な利用には Popen を使う
# - communicate の timeout 引数を使って、デッドロックや宙ぶらり状態の子プロセスを回避する
